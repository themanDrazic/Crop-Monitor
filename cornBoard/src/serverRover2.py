...
class RoverServer:
    ...
    def add_rover(self, addr):
        self.send_instruction(addr, "$SEND_ALL_DATA$")
        data = self.get_data(addr)
        roverName = data[0].strip('[').strip(']').strip('\'').split(',')
        connected = True
        self.rovers[roverName] = Rover(addr, connected)

    def get_rover_status(self, roverName):
        return self.rovers[roverName].is_connected()
...
class Rover:
    ...
    def __init__(self, addr, ID, connected):
        self.addr = addr
        self.ID = ID
